      ...
      {data.map((work) => (
        <div className="mt-6 flex gap-x-6" key={work.id}>
          ...
          <div className="flex-1">
            <h2 className="text-black dark:text-white capitalize font-bold text-base xl:text-lg">
              {work.role}
            </h2>
            ...
            {work.options && <div className="mt-2">
                {work?.options?.map((option,key) =>(
                    <li key={key} className="text-zinc-900 text-base font-semibold dark:text-zinc-400">{option}</li>
                ))}</div>}
          </div>
        </div>
      ))}
